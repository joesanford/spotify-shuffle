...
def sync_playlists(
    sp: Spotify, saved_tracks: list, playlist_tracks: list, destination_playlist: str
):
    ...
    if songs_to_remove:
        sp.playlist_remove_all_occurrences_of_items(
            destination_playlist, songs_to_remove
        )
...
def shuffle_playlist(sp: Spotify, tracks: list, playlist: str):
    ...
    while tracks_to_delete:
        sp.playlist_remove_all_occurrences_of_items(
            playlist, tracks_to_delete[:CHUNK_SIZE]
        )
        del tracks_to_delete[:CHUNK_SIZE]
        print(
            f"{round(100 - len(tracks_to_delete) / total_tracks_to_delete * 100, 1)}% deleted"
        )
